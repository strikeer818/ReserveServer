...
{
    public class JWTHandler(IConfiguration configuration, UserManager<ReservationCustomerUser> userManager)
    {
        // generate jwt token
        public async Task<JwtSecurityToken> GetTokenAsync(ReservationCustomerUser user) =>
        new(
        ...
        private async Task<List<Claim>> GetClaimsAsync(ReservationCustomerUser user)
        {
